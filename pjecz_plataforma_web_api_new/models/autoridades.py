...
class Autoridad(Base, UniversalMixin):
    ...
    organo_jurisdiccional: Mapped[str] = mapped_column(
        Enum(
            *ORGANOS_JURISDICCIONALES,
            name="autoridades_organos_jurisdiccionales",
            native_enum=False,
        ),
        index=True,
    )
    ...
    audiencia_categoria: Mapped[str] = mapped_column(
        Enum(
            *AUDIENCIAS_CATEGORIAS,
            name="autoridades_audiencias_categorias",
            native_enum=False,
        ),
        index=True,
    )
    ...
    # Hijos
    audiencias: Mapped[List["Audiencia"]] = relationship("Audiencia", back_populates="autoridad")
    edictos: Mapped[List["Edicto"]] = relationship("Edicto", back_populates="autoridad")
    glosas: Mapped[List["Glosa"]] = relationship("Glosa", back_populates="autoridad")
    ...
    tesis_jurisprudencias: Mapped[List["TesisJurisprudencia"]] = relationship("TesisJurisprudencia", back_populates="autoridad")
    ubicaciones_expedientes: Mapped[List["UbicacionExpediente"]] = relationship(
        "UbicacionExpediente", back_populates="autoridad"
    )

    @property
    def es_creador_glosas(self):
        """Es creador de glosas"""
        return self.organo_jurisdiccional in [
